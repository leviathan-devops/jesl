...
export const stateMachineNode: NodeImpl = {
  kind: "state-machine",
  ...
  invoke: (input: unknown, _ctx: unknown) =>
    Effect.gen(function* () {
      ...
      const cur = stateStore.get(key) ?? initial
      const startMs = yield* Clock.currentTimeMillis
      const next = table[cur]?.[event]
      if (!next) {
        const endMs = yield* Clock.currentTimeMillis
        return { verdict: "INCONCLUSIVE" as const, evidence: { pattern: "state-machine", state: cur, anchor: `${inp.node.id}:${cur}--${event}` }, timing: { startMs, endMs } } as NodeResult
      }
      stateStore.set(key, next)
      const verdict = next === "done" ? "PASS" : next === "failed" ? "FAIL" : "PASS"
      const endMs = yield* Clock.currentTimeMillis
      return { verdict: verdict as any, evidence: { pattern: "state-machine", state: next, anchor: `${inp.node.id}:${cur}->${next}` }, timing: { startMs, endMs }, outputs: { state: next, prev: cur } } as NodeResult
    })
}

export const _stateStore = stateStore
export const _defaultTable = defaultTable
