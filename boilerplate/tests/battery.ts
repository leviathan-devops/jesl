...
// Layout-independent: tests/ is INSIDE the kernel — fixtures/core are SIBLINGS of tests/.
// The old ROOT=../.. + ROOT/jesl/... hardwired the workspace layout and only passed on the
// host by directory coincidence (the container test exposed it: FIXTURES resolved to /root/jesl).
const JESL_ROOT = Path.resolve(__dirname, "..")
const FIXTURES = Path.join(JESL_ROOT, "fixtures")
...
async function scenarioS4(): Promise<BatteryRow> {
  ...
  try {
    const eff = Effect.gen(function* (): any {
      ...
      const now = Date.now()
      yield* partialJournal.append({ run: killRunId, node: "A", kind: "invoke", source: `workflow/kill-resume-fixture/A`, evidence: { pattern: "gate", state: "FIRED", anchor: "A:0" }, ts: now }).pipe(Effect.provide(partialLayer as any))
      yield* partialJournal.append({ run: killRunId, node: "A", kind: "verdict", verdict: "PASS", source: `workflow/kill-resume-fixture/A`, evidence: { pattern: "gate", state: "PASS", anchor: "A:1" }, ts: now + 1 }).pipe(Effect.provide(partialLayer as any))
...
async function scenarioS9(): Promise<BatteryRow> {
  ...
  try {
    const eff = Effect.gen(function* (): any {
      ...
      const h: NodeHandle = { invoke: () => Effect.gen(function* () { yield* Ref.update(c1, (n) => n + 1); const s = yield* Clock.currentTimeMillis; return { verdict: "PASS" as const, evidence: { pattern: "s9", state: "PASS", anchor: "A:1" }, timing: { startMs: s, endMs: s } }; }) } as any
