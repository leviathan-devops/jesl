...
describe("battery F23-F25", () => {
  ...
  it.effect("t3 determinism", () => Effect.gen(function* () {
    ...
    const makeH = (counter: any): any => ({ invoke: () => Effect.gen(function* () { yield* Ref.update(counter, (n: number) => n + 1); const s = yield* Clock.currentTimeMillis; return { verdict: PASS_TOK as "PASS", evidence: { pattern: "det", state: PASS_TOK, anchor: "mech:1" }, timing: { startMs: s, endMs: s }, outputs: { seed: { ok: 1 }, triplet: { state: PASS_TOK }, data: { ok: 1 } } }; }) })
  ...
  it.effect("t4 kill-resume", () => Effect.gen(function* () {
    ...
    const journal: any = yield* makeJournal
    const layer = Layer.succeed(Journal, journal)
    const fastHandle: any = { invoke: () => Effect.gen(function* () { const s = yield* Clock.currentTimeMillis; return { verdict: PASS_TOK as "PASS", evidence: { pattern: "kill", state: PASS_TOK, anchor: "k:1" }, timing: { startMs: s, endMs: s }, outputs: { ch1: { ok: 1 }, ch2: { ok: 1 }, ch3: { ok: 1 } } }; }) }
  ...
  it.effect("t5 replay zero", () => Effect.gen(function* () {
    ...
    const makeH = (): any => ({ invoke: () => Effect.gen(function* () { yield* Ref.update(counter, (n: number) => n + 1); const s = yield* Clock.currentTimeMillis; return { verdict: PASS_TOK as "PASS", evidence: { pattern: "s9", state: PASS_TOK, anchor: "s9:1" }, timing: { startMs: s, endMs: s }, outputs: { ch: { ok: 1 } } }; }) })
